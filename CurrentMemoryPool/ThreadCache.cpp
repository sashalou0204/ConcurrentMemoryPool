...
void* ThreadCache::Allocate(size_t memory_size)
{
	...
	memory_size = ClassSize::Roundup(memory_size);//计算对齐取整后的内存对象大小
	size_t index = ClassSize::Index(memory_size);//计算所需大小的内存对象所在链表在自由链表中的坐标
	FreeList* memory_list = &_freelist[index];

	if (!memory_list->Empty())//如果链表中有则直接从链表中Pop出一个就行
	{
		return memory_list->Pop();
	}
	else//如果链表中没有相应的对象则从central cache中申请
	{
